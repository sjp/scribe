...
- The `svg` renderer, writing the image with a selectable text layer over it:
  ...
  worked out from the whole of the layout — down to where each box sits, so
  that two crops of one sign, saying the same words over an image of the same
  size, are still named apart — given by the caller or left out; and a prefix
  of nothing with no token to follow it is refused, since the names would then
  be as bare as `root`; every rule in its stylesheet hangs off the root element
